...
| OWRT-ALL | MEDIATEK-WIFI-YES、ROCKCHIP、X86 | VIKINGYFY/immortalwrt `owrt` | Argon，192.168.10.1 |
...
默认主机名为 `OWRT`，工作流中的登录密码提示为“无”。Auto-Clean 每天北京时间 06:00 运行，成功完成后触发 QCA-ALL 和 OWRT-ALL；实际开始时间受 GitHub Actions 调度影响。固件页面显示编译开始时间。

## 手动编译和配置检查

在 Actions 中运行 `QCA-ALL` 可构建太乙/M2 固件；勾选 `TEST` 时只生成配置文件。也可运行 `WRT-TEST`，默认选择 `IPQ60XX-WIFI-NO`、`VIKINGYFY/immortalwrt`、`main`，且默认仅输出配置。

使用 VIKINGYFY 源码时，IPQ60XX/IPQ807X 选择 `main`；IPQ53XX/IPQ95XX、MediaTek、Rockchip、X86 选择 `owrt`。其他源码仓库需要选择其实际支持的配置和分支。
...
- `Config/`：设备及通用 Kconfig。原 `MEDIATEK` 拆为 `MEDIATEK-WIFI-YES` / `MEDIATEK-WIFI-NO`；旧 IPQ50XX 配置退出手动菜单，新 IPQ53XX/IPQ95XX 使用各自配置，不能互换。
- `Config/TEST.txt`：太乙/M2 无 Wi-Fi 的独立测试配置，包含通用设置；修改默认配置时需同步更新。
...
可选扩展入口为 `Scripts/PRIVATE.sh` 和 `Config/PRIVATE.txt`。Kconfig 应用顺序是设备配置 → GENERAL（TEST 除外）→ 默认主题 → PRIVATE → PACKAGE；后面的配置可覆盖前面的同名选项。
